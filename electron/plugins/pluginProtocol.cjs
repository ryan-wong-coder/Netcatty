...
function htmlForRuntime(token) {
  const sdk = `${PLUGIN_PROTOCOL_SCHEME}://${token}/__host/sdk/index.js`;
  const contract = `${PLUGIN_PROTOCOL_SCHEME}://${token}/__host/contract/index.js`;
...
<script type="importmap" nonce="${token}">${JSON.stringify({ imports: {
    "@netcatty/plugin-sdk": sdk,
    "@netcatty/plugin-contract": contract,
  } })}</script></head><body><script type="module" src="${runtime}"></script></body></html>`;
}
  ...
  async #handle(request) {
    ...
    try {
      ...
      if (segments.length === 1 && segments[0] === "index.html") {
        return respond(htmlForRuntime(registration.token), 200, "text/html; charset=utf-8", {
      ...
      } else if (segments[0] === "__host" && segments[1] === "sdk") {
        root = this.sdkDirectory;
        fileSegments = segments.slice(2);
      } else if (segments[0] === "__host" && segments[1] === "contract") {
        root = this.contractDirectory;
        fileSegments = segments.slice(2);
      } else return respond("Not Found", 404);
      if (!fileSegments.length || fileSegments[0].startsWith(".")) return respond("Not Found", 404);
