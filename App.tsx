...
import { applyEphemeralHostsUpdate, splitHostsUpdateByEphemeral, upsertEphemeralHost } from './domain/ephemeralHosts';
...
function App({ settings }: { settings: SettingsState }) {
  ...
  const handleConnectToEphemeralHost = useCallback((host: Host) => {
    setEphemeralHosts((previous) => upsertEphemeralHost(previous, host));
    return handleConnectToHost(host);
  }, [handleConnectToHost]);
